...
// Pemasukan User
try{
$query2 = $conn->query("SELECT  SUM(pemasukan) AS masuk FROM dolar WHERE tgl BETWEEN '$dari' AND '$sampai' AND jenis = 'masuk' AND jeniss='BANPROV' "); 
...
// Pengeluaran User
$query3 = $conn->query("SELECT  SUM(pengeluaran) AS keluar FROM dolar WHERE tgl BETWEEN '$dari' AND '$sampai' AND jenis = 'keluar' AND jeniss='BANPROV' ");
$data2 = $query3->fetch(PDO::FETCH_OBJ);
$jumlah2 =  $data2->keluar;
$resultJumlahPengeluaran = number_format($jumlah2,2,',','.');

// Sisa Saldo / Uang User 
$query4 = $conn->query("SELECT ROUND ( SUM(IF(jenis = 'masuk' AND jeniss='BANPROV', pemasukan, 0))-(SUM(IF( jenis = 'keluar' AND jeniss='BANPROV', pengeluaran, 0))) ) AS subtotal FROM dolar WHERE tgl BETWEEN '$dari' AND '$sampai'");
...
header("Content-Disposition: attachment; filename=CETAK BANPROV.xls");
?>


<h1>&nbsp;&nbsp;LAPORAN Banprov</h1>
...
$no = 1;
    ...
    $sql = "select * from dolar where tgl BETWEEN '$dari' AND '$sampai' and jeniss='BANPROV'  ";
