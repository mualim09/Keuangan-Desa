...
// Pemasukan User
try{
$query2 = $conn->query("SELECT  SUM(pemasukan) AS masuk FROM dolar WHERE tgl BETWEEN '$dari' AND '$sampai' AND jenis = 'masuk' AND jeniss='ADD' "); 
...
$query4 = $conn->query("SELECT ROUND ( SUM(IF(jenis = 'masuk' AND jeniss='ADD', pemasukan, 0))-(SUM(IF( jenis = 'keluar' AND jeniss='ADD', pengeluaran, 0))) ) AS subtotal FROM dolar WHERE tgl BETWEEN '$dari' AND '$sampai'");
...
<div class="row">
    ...
    <?php
     $no = 1;
    $sql = "select * from dolar where tgl BETWEEN '$dari' AND '$sampai' and jeniss='ADD'  ";
