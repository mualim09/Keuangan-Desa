...
function tambah($koneksi){
        ...
        $id = time();
         $kode = $_POST['kode'];
          $keterangan = $_POST['keterangan'];
           $tgl = $_POST['tgl'];
            $banprovm = $_POST['banprovm'];
            

           if(!empty($banprovm)){
         error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
           $sql = "INSERT INTO dolar (id, kode, keterangan, tgl, banprovm, jenis, jeniss) VALUES(".$id.",'".$kode."','".$keterangan."','".$tgl."','".$banprovm."','".masuk."','".BANPROV."')";
 ...
 if($sql) {
            ...
            if($simpan && isset($_GET['aksi'])){
             ...
             echo "
                                <script>
                                alert('Data BANPROV Tidak Boleh Kosong');
                                document.location.href = '';
                                </script>"; 
        ...
        <form action="apa2.php" method="POST" enctype="multipart/form-data">
            ...
            <fieldset>
                <legend><h2>Tambah Data</h2></legend>
                  ...
                  <label for="pemasukan">Jumlah Pemasukan BANPROV </label><input type="number" class="form-control" name="pemasukan"/>
...
include '../koneksi.php';}
                                    ...
                                    $no = 1;
                                    $sql = mysqli_query($koneksi, "SELECT * FROM dolar WHERE pemasukan AND jeniss='BANPROV' AND jenis = 'masuk' ");
                                      ...
                                      <tr class="odd gradeX">
                                           ...
                                           <td>
                                            ...
                                            <td><a href="foto/<?php echo $data['foto']; ?>" class="perbesar"><img src="foto/<?php echo $data['foto']; ?>" width="50"/></td>
                                                                           ...
                                                                           <td>
                                                 <a class="btn btn-primary"  href="banprov.php?aksi=update&id=<?php echo $data['id']; ?>&kode=<?php echo $data['kode']; ?>&pembuat=<?php echo $data['pembuat']; ?>&keterangan=<?php echo $data['keterangan']; ?>&tgl=<?php echo $data['tgl']; ?>&pemasukan=<?php echo $data['pemasukan']; ?>&foto=<?php echo $data['foto'];?>">Ubah</a> |
                    <a class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus data?')" href="banprov.php?aksi=delete&id=<?php echo $data['id']; ?>">Hapus</a>
                                ...
                                <tr>
                                  <?php error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));  ?>
                                    <td colspan="5" style="text-align: left; font-size: 17px; color: maroon;">Total Kas BANPROV IN :</td>
  ...
  if($foto != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
                  ...
                  $perubahan = "kode='".$kode."',pembuat='".$pembuat."',keterangan='".$keterangan."',tgl='".$tgl."',pemasukan='".$pemasukan."',foto='".$nama_gambar_baru."'";
                   ...
                   $query  = "UPDATE dolar SET ".$perubahan." WHERE id=$id";
                    ...
                    if(!$result){
                      ...
                      echo "<script>alert('Data berhasil diubah.');window.location='banprov.php';</script>";
                    }
              } else {     
               //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                  echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='banprov.php';</script>";
              }
    } else {
      // jalankan query UPDATE berdasarkan ID yang produknya kita edit
      ...
      if(!$result){
        ...
        //tampil alert dan akan redirect ke halaman index.php
        //silahkan ganti index.php sesuai halaman yang akan dituju
          echo "<script>alert('Data .');window.location='banprov.php';</script>";
      }
    }
    ...
    if(isset($_GET['id'])){
        ?>
            ...
            <fieldset>
              <a href="banprov.php"><button type="button" class="close" data-dismiss="modal" aria-label="Close" ><span aria-hidden="true">&times;</span></button></a>
                ...
                <input type="hidden" name="id" value="<?php echo $_GET['id'] ?>"/>
                  ...
                  <label for="pembuat">Penerima Keuangan </label><input type="text" class="form-control" name="pembuat" value="<?php echo $_GET['pembuat'] ?>" readonly="readonly"/>
                  ...
                  <label for="pemasukan">Jumlah Pemasukan BANPROV </label><input type="number" class="form-control" name="pemasukan" value="<?php echo $_GET['pemasukan'] ?>"/>
                   <div>
               ...
               error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
                ...
                if($hapus) {
                        echo "
                                ...
                                document.location.href = 'banprov.php';
                                </script>";  
}else{ 
    echo "
                                <script>
...
if (isset($_GET['aksi'])){
    switch($_GET['aksi']){
        case "create":
            echo '<a href="banprov.php"> &laquo; Home</a>';
            tambah($koneksi);
            break;
        case "read":
            cari($koneksi);
