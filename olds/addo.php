    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if(!empty($addo)){
         error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
           $sql = "INSERT INTO dolar (id, kode, keterangan, tgl, addo, jenis, jeniss) VALUES(".$id.",'".$kode."','".$keterangan."','".$tgl."','".$addo."','".keluar."','".ADD."')";
        ...
        <form action="apa1.php" method="POST" enctype="multipart/form-data">
            ...
            <fieldset>
                <legend><h2>Tambah Data</h2></legend>
                  ...
                  <label for="pengeluaran">Jumlah Pengeluaran ADD </label><input type="number" class="form-control" name="pengeluaran"/>
...
include '../koneksi.php';}
                                    ...
                                    $no = 1;
                                    $sql = mysqli_query($koneksi, "SELECT * FROM dolar WHERE pengeluaran AND jeniss='ADD' AND jenis = 'keluar' ");
                                ...
                                ?>
                                      <tr class="odd gradeX">
                                          ...
                                          <td>
                                            ...
                                            <td>
                                                 <a class="btn btn-primary"  href="addo.php?aksi=update&id=<?php echo $data['id']; ?>&kode=<?php echo $data['kode']; ?>&pembuat=<?php echo $data['pembuat']; ?>&keterangan=<?php echo $data['keterangan']; ?>&tgl=<?php echo $data['tgl']; ?>&pengeluaran=<?php echo $data['pengeluaran']; ?>&foto=<?php echo $data['foto'];?>">Ubah</a>|
                    <a class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus data?')" href="addo.php?aksi=delete&id=<?php echo $data['id']; ?>">Hapus</a>
                                            </td>
                                ...
                                <tr>
                                  <?php error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));  ?>
                                    <td colspan="5" style="text-align: left; font-size: 17px; color: maroon;">Total Kas ADD :</td>
  ...
  if($foto != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
                  ...
                  $perubahan = "kode='".$kode."',pembuat='".$pembuat."',keterangan='".$keterangan."',tgl='".$tgl."',pengeluaran='".$pengeluaran."',foto='".$nama_gambar_baru."'";
                   ...
                   $query  = "UPDATE dolar SET ".$perubahan." WHERE id=$id";
                    ...
                    if(!$result){
                      ...
                      echo "<script>alert('Data berhasil diubah.');window.location='addo.php';</script>";
                    }
              } else {     
               //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                  echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='addo.php';</script>";
              }
    } else {
      // jalankan query UPDATE berdasarkan ID yang produknya kita edit
      ...
      if(!$result){
        ...
        //tampil alert dan akan redirect ke halaman index.php
        //silahkan ganti index.php sesuai halaman yang akan dituju
          echo "<script>alert('Data .');window.location='addo.php';</script>";
      }
    }
            ...
            <fieldset>
              <a href="addo.php"><button type="button" class="close" data-dismiss="modal" aria-label="Close" ><span aria-hidden="true">&times;</span></button></a>
                ...
                <input type="hidden" name="id" value="<?php echo $_GET['id'] ?>"/>
                  ...
                  <label for="pengeluaran">Jumlah Pengeluaran ADD </label><input type="number" class="form-control" name="pengeluaran" value="<?php echo $_GET['pengeluaran'] ?>"/>
