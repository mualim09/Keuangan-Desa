    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if(!empty($banprovo)){
         error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
           $sql = "INSERT INTO dolar (id, kode, keterangan, tgl, banprovo, jenis, jeniss) VALUES(".$id.",'".$kode."','".$keterangan."','".$tgl."','".$banprovo."','".keluar."','".BANPROV."')";
  ...
  <div id="tambahuser" class="modal fade" role="dialog" style="display:none;">
    <div class="modal-dialog"> 
      <div class="modal-content">
        ...
        <form action="apa2.php" method="POST" enctype="multipart/form-data">
            ...
            <fieldset>
                <legend><h2>Tambah Data</h2></legend>
                  ...
                  <label for="pembuat">Penjual </label><input type="text" class="form-control" name="pembuat" />
                  <label for="pengeluaran">Jumlah Pengeluaran BANPROV </label><input type="number" class="form-control" name="pengeluaran"/>
                 
              <label for="foto">Kwitansi</label>
            <input type="file" name="foto" value="" required=""></form>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12"><h4>PENGELUARAN</h4>
                    <h1 class="page-header">Tables BANPROV ( BANTUAN PROVINSI )</h1>
                </div>
...
include '../koneksi.php';}
                                    ...
                                    $no = 1;
                                    $sql = mysqli_query($koneksi, "SELECT * FROM dolar WHERE pengeluaran AND jeniss='BANPROV' AND jenis = 'keluar' ");
                                      ...
                                      <tr class="odd gradeX">
                                          ...
                                          <td align="right">
                                            ...
                                            <td>
                                             ...
                                             <td align="right">
                                                <?php echo number_format($data['pengeluaran']).",-"; ?>
                                            </td>
                                            ...
                                            <td>
                                                 <a class="btn btn-primary"  href="banprovo.php?aksi=update&id=<?php echo $data['id']; ?>&kode=<?php echo $data['kode']; ?>&pembuat=<?php echo $data['pembuat']; ?>&keterangan=<?php echo $data['keterangan']; ?>&tgl=<?php echo $data['tgl']; ?>&pengeluaran=<?php echo $data['pengeluaran']; ?>&foto=<?php echo $data['foto'];?>">Ubah</a>|
                    <a class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus data?')" href="banprovo.php?aksi=delete&id=<?php echo $data['id']; ?>">Hapus</a>
                                            </td>
                                ...
                                <tr>
                                  <?php error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));  ?>
                                    <td colspan="5" style="text-align: left; font-size: 17px; color: maroon;">Total BANPROV :</td>
...
function ubah($koneksi){
  ...
  if($foto != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
                  ...
                  $perubahan = "kode='".$kode."',pembuat='".$pembuat."',keterangan='".$keterangan."',tgl='".$tgl."',pengeluaran='".$pengeluaran."',foto='".$nama_gambar_baru."'";
                   ...
                   $query  = "UPDATE dolar SET ".$perubahan." WHERE id=$id";
                    ...
                    if(!$result){
                      ...
                      echo "<script>alert('Data berhasil diubah.');window.location='banprovo.php';</script>";
                    }
              } else {     
               //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                  echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='banprovo.php';</script>";
              }
    } else {
      // jalankan query UPDATE berdasarkan ID yang produknya kita edit
         $perubahan = "kode='".$kode."',pembuat='".$pembuat."',keterangan='".$keterangan."',tgl='".$tgl."',pengeluaran='".$pengeluaran."'";
      $query  = "UPDATE dolar SET ".$perubahan." WHERE id=$id";
      $result = mysqli_query($koneksi, $query);
      // periska query apakah ada error
      if(!$result){
        ...
        //tampil alert dan akan redirect ke halaman index.php
        //silahkan ganti index.php sesuai halaman yang akan dituju
          echo "<script>alert('Data .');window.location='banprovo.php';</script>";
      }
    }
    ...
    if(isset($_GET['id'])){
        ?>
            ...
            <fieldset>
              <a href="banprovo.php"><button type="button" class="close" data-dismiss="modal" aria-label="Close" ><span aria-hidden="true">&times;</span></button></a>
                ...
                <input type="hidden" name="id" value="<?php echo $_GET['id'] ?>"/>
                  ...
                  <label for="pembuat">Penjual</label><input type="text" class="form-control" name="pembuat" value="<?php echo $_GET['pembuat'] ?>" readonly="readonly"/>
                  ...
                  <label for="pengeluaran">Jumlah Pengeluaran BANPROV </label><input type="number" class="form-control" name="pengeluaran" value="<?php echo $_GET['pengeluaran'] ?>"/>
          ...
          <i style="float: left;font-size: 11px;color: red">Abaikan jika tidak merubah gambar produk</i>
                  ...
                  <br>
                <label>
                    <input type="submit" name="btn_ubah" class="btn btn-primary" value="Simpan Perubahan"/>  atau <a  class="btn btn-danger" href="banprovo.php">Batal</a>
                </label>
                <br>
                <p><?php echo isset($pesan) ? $pesan : "" ?></p>
               ...
               error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
                ...
                if($hapus) {
                        echo "
                                ...
                                document.location.href = 'banprovo.php';
                                </script>";  
}else{ 
    echo "
                                <script>
...
if (isset($_GET['aksi'])){
    switch($_GET['aksi']){
        case "create":
            echo '<a href="banprovo.php"> &laquo; Home</a>';
            tambah($koneksi);
            break;
        case "read":
            cari($koneksi);
